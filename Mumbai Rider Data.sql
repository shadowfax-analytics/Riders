...
where d.cluster_name not like "%test%" and attendancedate = adddate(curdate(),-1) and b.status = 1 and d.city = 'BOM') as tab1 left outer join
...
select * from coreengine_ridersession where id in (select a.id as id from coreengine_ridersession a,coreengine_riderattendance b where a.rider_id = b.rider_id and date(convert_tz(a.in_time,"UTC","Asia/Kolkata")) = adddate(curdate(),-1) and date(convert_tz(b.attendancedate,"UTC","Asia/Kolkata")) = adddate(curdate(),-1) order by a.rider_id,abs(convert_tz(a.in_time,"UTC","Asia/Kolkata") - convert_tz(b.expected_intime,"UTC","Asia/kolkata"))) group by rider_id) as intt inner join 
(select * from coreengine_ridersession where id in (select a.id as id from coreengine_ridersession a,coreengine_riderattendance b where a.rider_id = b.rider_id and date(convert_tz(a.in_time,"UTC","Asia/Kolkata")) = adddate(curdate(),-1) and date(convert_tz(b.attendancedate,"UTC","Asia/Kolkata")) = adddate(curdate(),-1) order by a.rider_id,abs(convert_tz(a.out_time,"UTC","Asia/Kolkata") - convert_tz(b.expected_outtime,"UTC","Asia/kolkata"))) group by rider_id) as outt on intt.rider_id = outt.rider_id) 
